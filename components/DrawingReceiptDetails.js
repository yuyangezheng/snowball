...
const DrawingReceiptDetails = ({ receiptId }) => {
  ...
  const { runContractFunction: getDrawingReceipt } = useWeb3Contract({
    abi: drawingManagerABI,
    contractAddress: contractAddresses[wallet.chainId]?.drawingManager[0],
    functionName: "DrawingReceipts",
    params: { "": receiptId },
  });
  ...
  const { runContractFunction: getNonERCDrawingOwner } = useWeb3Contract({
    abi: drawingManagerABI,
    contractAddress: contractAddresses[wallet.chainId]?.drawingManager[0],
  ...
  const handleDrawingReceipt = async () => {
    try {
      console.log("Hi");
      const result = await getDrawingReceipt();
      console.log(result);
      setReceiptDetails({
        drawingID: result.drawingID,
        redeemableAmount: result.redeemableAmount,
      });
      const tokenOwner = await getERC721Owner();
      console.log(tokenOwner);
      setReceiptOwnerERC(tokenOwner);
      const nonTokenOwner = await getNonERCDrawingOwner();
      ...
      setError("Failed to fetch drawing receipt.");
    }
  };
  ...
  return (
    <div className={styles.receiptDetails}>
      ...
      {receiptDetails && (
        <div>
          <h2>Drawing Receipt Details</h2>
          <p>
            <strong>Drawing ID:</strong>{" "}
            {ethers.utils.formatUnits(receiptDetails.drawingID, 0)}
          </p>
          <p>
            <strong>Redeemable Amount Amount:</strong>{" "}
            {ethers.utils.formatUnits(receiptDetails.redeemableAmount, 6)} ETH
          </p>
          <p>
...
export default DrawingReceiptDetails;
