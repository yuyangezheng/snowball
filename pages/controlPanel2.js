...
export default function Test() {
  ...
  const { runContractFunction: getSnowballEarnedCommissions } = useWeb3Contract(
    {
  ...
  const { runContractFunction: getSnowballWithdrawnCommissions } =
    useWeb3Contract({
      abi: snowballManagerABI,
      contractAddress: contractAddresses[wallet.chainId]?.snowballManager[0],
  ...
  const { runContractFunction: balanceOf } = useWeb3Contract({
    ...
    params: {
      account: contractAddresses[wallet.chainId]?.snowballManager[0],
    },
  });
  ...
  const fetchCommissions = async () => {
    ...
    try {
      console.log("Getting Snowball Earned Commissions");
      const snowballEarned = await getSnowballEarnedCommissions();
      console.log(snowballEarned);
      setEarnedCommissions(snowballEarned);

      console.log("Getting Snowball Withdrawn Commissions");
      const snowballWithdrawn = await getSnowballWithdrawnCommissions();
      console.log(snowballWithdrawn);
      setWithdrawnCommissions(snowballWithdrawn);
  ...
  return (
    <div>
      <ApprovedCallers />
      <h1>Commissions and Manager Details for Snowballs</h1>
      {loading && <p>Loading...</p>}
