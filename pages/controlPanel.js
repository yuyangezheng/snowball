...
export default function Test() {
  ...
  const { runContractFunction: getSeedEarnedCommissions } = useWeb3Contract({
    abi: seedManagerABI,
  ...
  const { runContractFunction: getSeedWithdrawnCommissions } = useWeb3Contract({
    abi: seedManagerABI,
    contractAddress: contractAddresses[wallet.chainId]?.seedManager[0],
  ...
  const { runContractFunction: balanceOf } = useWeb3Contract({
    ...
    params: {
      account: contractAddresses[wallet.chainId]?.seedManager[0],
    },
  });
  ...
  const fetchCommissions = async () => {
    ...
    try {
      console.log("Getting Seed Earned Commissions");
      const seedEarned = await getSeedEarnedCommissions();
      console.log(seedEarned);
      setEarnedCommissions(seedEarned);

      console.log("Getting Seed Withdrawn Commissions");
      const seedWithdrawn = await getSeedWithdrawnCommissions();
      console.log(seedWithdrawn);
      setWithdrawnCommissions(seedWithdrawn);
  ...
  return (
    <div>
      <ApprovedCallers />
      <h1>Commissions and Manager Details for Seeds</h1>
      {loading && <p>Loading...</p>}
